...
class Compose(object):
    def __init__(self, trans_forms):
        self.trans_forms= trans_forms

    def __call__(self, img, anno_class_img):
        for t in self.trans_forms:
...
class Scale(object):
    ...
    def __call__(self, img, anno_class_img):
        ...
        if scale >1.0:
            ...
            # sau khi scale => crop anh dung ve kich thuoc ban dau
...
class Resize(object):
    ...
    def __call__(self, img, anno_class_img):
        ...
        anno_class_img= anno_class_img.resize((self.input_size, self.input_size), Image.BILINEAR)

        return img, anno_class_img
# chuyen img tu np sang tensor
class Normalize_Tensor(object):
    ...
    def __call__(self, img, anno_class_img):
        ...
        # mot so anh phan segmentation khong chac chan ne dc to mau trang => can chuyen sang mau den
        anno_class_img= np.array(anno_class_img)

        index= np.where(anno_class_img==256)
